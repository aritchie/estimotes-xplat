...
namespace NearableExample
{
	partial class TriggersViewController : UIViewController, IUIPickerViewDataSource, IUIPickerViewDelegate
	{
		...
		private void StartReminderTrigger()
		{
			...
			var insideCarRule = ProximityRule.InRangeOfNearable (NearableType.Car);
			var noBagRule = ProximityRule.OutsideRangeOfNearable (NearableType.Bag);
		...
		void HandleTriggerChangedState (object sender, TriggerChangedStateArgsEventArgs e)
		{
		...
		private void StopReminderTrigger()
		{
			...
			triggerManager.StopMonitoring (TriggerId);
			triggerManager.ChangedState -= HandleTriggerChangedState;
